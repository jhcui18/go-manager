-- 034: shipping_status items carry the EMS basis so the joiner's receipt can show
-- the same breakdown as the admin packing slip: 'cards' (photocards at the flat
-- per-card rate — same rule as the queue: photocard-type GO or POB name contains
-- "photocard"; qty each) and 'ems' (a non-card item's per-unit EMS × qty, null
-- when the admin never entered one). Shop orders never count. Body otherwise as 020.
create or replace view shipping_status as
  select r.id, r.username, r.ems_fee, r.dom_fee, r.total_fee, r.shipped, r.created_at,
         (select coalesce(jsonb_agg(jsonb_build_object(
             ...
             'cards', case when i.claim_id is not null and (g.type = 'photocard' or si.name ilike '%photocard%')
                             then coalesce(c.qty, 1) else 0 end,
             'ems', case when i.claim_id is not null and not (g.type = 'photocard' or si.name ilike '%photocard%')
                              and si.ems_fee is not null
                             then si.ems_fee * coalesce(c.qty, 1) else null end) order by i.id), '[]'::jsonb)
            from shipping_request_items i
            left join claims c on c.id = i.claim_id
            left join sub_items si on si.id = c.sub_item_id
            left join gos g on g.id = si.go_id
