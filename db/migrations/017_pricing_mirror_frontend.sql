-- 017: the set-vs-claims pricing split must mirror the frontend EXACTLY. 016
-- used sub_items.order_mode, but the client shapes POBs by GO type + kind
-- (index.html rebuild): photocard GOs, album GOs' member POBs, and merch GOs'
-- member-set POBs are set-shaped (slots at flat price, OT at ot_price); merch
-- GOs' member POBs are claims-based (price*qty) even though their DB rows say
-- order_mode='set'. 276 claims sat in that divergent combo. Also realigns the
-- go_money view (013) with the same rule: set slots price FLAT (the app ignores
-- qty on slots), claims-based rows price*qty.
