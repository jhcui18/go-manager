-- 033: apply_balance applies credit PARTIALLY. Previously credit moved only when
-- it completed at least one whole unpaid item (budget = balance + loose cash),
-- so a joiner whose credit was smaller than every unpaid item on a GO could not
-- use it at all. Now the balance is drawn up to the GO's remaining owed amount
-- regardless; whole items still flip to paid when fully covered, and the rest
-- lowers Owed as loose cash (confirm_payment later completes the item).
-- Body otherwise identical to 022.
...
begin
  ...
  loop
    ...
    v_unpaid_total := v_unpaid_total + v_unit.value;
  ...
  -- 033: PARTIAL application. Draw the whole balance up to what this GO still
  -- owes (unpaid units minus loose cash), even when that finishes no single
  -- item — the remainder sits as loose cash on the target GO, lowering Owed;
  -- items flip to paid only when fully covered (greedy above). Before this a
  -- credit smaller than every unpaid item could never be used at all.
  v_need := least(v_balance, greatest(0, v_unpaid_total - v_loose));
  if v_need <= 0.001 and coalesce(array_length(v_covered_ids, 1), 0) = 0 then
