-- 015: apply_balance learns about loose cash on the target GO. Previously an
-- unpaid unit was only coverable when the cross-GO credit ALONE covered its full
-- price, so a joiner who part-paid an item in cash (e.g. $4 toward a $6 card)
-- could never finish it with a smaller credit ($2). The spend budget is now
-- credit + unallocated confirmed cash on the target GO (confirmed payments minus
-- the value of units already marked paid); the amount DRAWN from source GOs is
-- still capped at what the loose cash doesn't cover, so the ledger stays exact.
-- Everything else (unit ordering, greedy whole-unit covering, source draws)
-- matches 008 unchanged.
...
begin
  ...
  with owed_t as (
    select si.price as value, (c.payment_status = 'paid') as is_paid
      from claims c join sub_items si on si.id = c.sub_item_id
     where c.username = v_username and si.go_id = p_go_id and c.set_id is not null
       and c.is_ot = false and c.status = 'secured'
    union all
    select si.ot_price, bool_and(c.payment_status = 'paid')
      from claims c join sub_items si on si.id = c.sub_item_id
     where c.username = v_username and si.go_id = p_go_id and c.set_id is not null
       and c.is_ot = true and c.status = 'secured'
     group by c.set_id, si.ot_price
    union all
    select si.price * c.qty, (c.payment_status = 'paid')
      from claims c join sub_items si on si.id = c.sub_item_id
     where c.username = v_username and si.go_id = p_go_id and c.set_id is null
