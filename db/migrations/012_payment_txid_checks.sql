-- 012: payment submissions must carry a transaction id, and ids that collide
-- with earlier payments are rejected server-side:
--   * txid_required   — blank/missing transaction id
--   * txid_other_user — id matches another user's CONFIRMED payment
--   * txid_duplicate  — same user already submitted this id for the same GO
--                       (or the shop), whatever its status
-- Reuse of one transfer's id by the SAME user across DIFFERENT GOs stays
-- allowed — the rules explicitly tell multi-GO payers to do that.
