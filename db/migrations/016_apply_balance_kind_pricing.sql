-- 016: price claims the way the frontend does. apply_balance split set-based vs
-- claims-based units on "set_id is not null", but the frontend's discriminator is
-- the POB's kind: only member/photocard/member-set POBs in set mode with a member
-- and set are priced as flat-price slots; everything else is price*qty. A
-- versioned merch claim that happened to carry a set_id (e.g. qty 2 x $12) was
-- priced flat ($12), undercounting secured value and minting phantom credit --
-- which apply_balance then happily drew from the wrong GO.
  ...
  owed as (
    select si.go_id, si.price as value, (c.payment_status = 'paid') as is_paid
     ...
     where c.username = v_username and c.status = 'secured' and c.is_ot = false
       and si.kind in ('member','photocard','member-set') and si.order_mode = 'set'
    ...
    select si.go_id, si.price * c.qty, (c.payment_status = 'paid')
     ...
     where c.username = v_username and c.status = 'secured'
       and not (si.kind in ('member','photocard','member-set') and si.order_mode = 'set'
                and c.member_id is not null and c.set_id is not null)
  ),
  go_secured as (
