-- 018: per-claim price override. claims.price_override (null = item price)
-- replaces the item price wherever a single claim is valued: flat for
-- set-shaped slots, x qty for claims-based rows. OT full-set groups are
-- deliberately excluded (an OT set prices as ONE unit at ot_price; per-row
-- overrides would be ambiguous). apply_balance and go_money updated; the
-- frontend mirrors the same rule.
alter table claims add column if not exists price_override numeric(10,2);
...
with sec as (
  select si.go_id, si.id as si_id,
         coalesce(c.price_override, si.price, 0) as price, coalesce(si.ot_price, 0) as ot_price,
