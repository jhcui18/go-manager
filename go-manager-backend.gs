...
const SHEET_SUBITEM_PAYDUE = 'subitem_payment_due'; // per-POB payment due dates ("Pay by")
...
function bootstrapSheets() {
  ...
  ensureSheet(ss, SHEET_SUBITEM_PAYDUE, ['go_id','sub_item_id','due_date']);
...
function doPost(e) {
  ...
  try {
    ...
    else if (action === 'setSubItemPayDue')    result = setSubItemPayDue(body.data);
...
function deleteGO(goId) {
  ...
  deleteRowsWhere(ss.getSheetByName(SHEET_SUBITEM_PAYDUE), 'go_id', goId);
...
function getSubItemPayDue() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_SUBITEM_PAYDUE);
  return { subitem_payment_due: sheet ? sheetToObjects(sheet) : [] };
}

function setSubItemPayDue(data) {
  bootstrapSheets();
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_SUBITEM_PAYDUE);
  const rows = sheet.getDataRange().getValues();
  const h = rows[0];
  const gi = h.indexOf('go_id'), si = h.indexOf('sub_item_id'), di = h.indexOf('due_date');
  let foundRow = -1;
  for (let i = 1; i < rows.length; i++) {
    if (rows[i][gi] === data.go_id && rows[i][si] === data.sub_item_id) { foundRow = i; break; }
  }
  ...
  if (due) {
    if (foundRow === -1) sheet.appendRow([data.go_id, data.sub_item_id, due]);
